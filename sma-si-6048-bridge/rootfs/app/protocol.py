...
class CellVoltagesFrame1:
    ...
    Each voltage: 0.001V per LSB (11-bit, 5-bit unused)
    """
    ...
    def pack(cell_voltages: Tuple[float, float, float, float]) -> bytes:
        ...
        for i, voltage in enumerate(cell_voltages[:4]):
            raw = int(voltage / 0.001) & 0x7FF  # 11-bit
            offset = i * 2
            data[offset] = raw & 0xFF
            data[offset + 1] = ((raw >> 8) & 0x07)
        return bytes(data)
    ...
    def unpack(data: bytes) -> Dict:
        ...
        for i in range(4):
            offset = i * 2
            raw = data[offset] | ((data[offset + 1] & 0x07) << 8)
...
def encode_frame(frame_id: int, payload: Dict) -> bytes:
    ...
    if encoder_class:
        return encoder_class.pack(**payload)
